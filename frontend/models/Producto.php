...
class Producto extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['categoria_id', 'coleccion_id', 'nombre'], 'required'],
            [['categoria_id', 'coleccion_id', 'precio_id', 'created_at', 'updated_at'], 'integer'],
            [['descripcion'], 'string'],
    ...
    public function tallas($id, $array = false){
        if($array)
            return ArrayHelper::map(
                    Producto::find()
                        ->select(['talla.id AS id', 'talla.talla AS nombre'])
    ...
    public function colores($id, $array = false){
        if($array)
            return ArrayHelper::map(
                    Producto::find()
                        ->select(['color.id AS id', 'color.color AS nombre'])
    ...
    public function fotoCorreo($id){
        return $this->find()
        ->select('producto.nombre,foto.archivo AS foto')
        ->join('INNER JOIN', 'foto', 'foto.producto_id = producto.id')
        ->where('producto.id='.$id)
        ->one();
    }
